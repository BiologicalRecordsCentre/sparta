...
temp <- capture.output(weights <- createWeights(dist, sim, dist_sub = 5, sim_sub = 3))
...
test_that("Test errors and warnings", {
  ...
  expect_error(createWeights(dist, sim, dist_sub = 5, sim_sub = 10),
               "dist_sub' cannot be smaller than or equal to 'sim_sub'") 
  expect_error(createWeights(TRUE, sim, dist_sub = 5, sim_sub = 3),
               "dist must be a data.frame") 
  expect_error(createWeights(dist, 'Tom', dist_sub = 5, sim_sub = 3),
               "sim must be a data.frame") 
  expect_error(createWeights(cbind(dist, 1:nrow(dist)), sim, dist_sub = 5, sim_sub = 3),
               "dist must have three columns")
  expect_error(createWeights(cbind(dist[,1:2], as.character(dist[,3])), sim, dist_sub = 5, sim_sub = 3),
               "the value column in dist must be an integer or numeric") 
  expect_error(createWeights(dist, cbind(sim[1:10], as.character(sim[,11])), dist_sub = 5, sim_sub = 3),
