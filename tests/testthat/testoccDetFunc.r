...
# format data
suppressWarnings({
  visitData_missing <- formatOccData(
    taxa = taxa, site = site,
    survey = time_period_missing
  )
})
suppressWarnings({
  visitData_missing2 <- formatOccData(
    taxa = taxa, site = site,
...
suppressWarnings({
  visitData_filterError <- formatOccData(
    taxa = taxa_filterError, site = site_filterError,
...
test_that("Test occDetFunc using regions and region aggregates", {
  ...
  expect_error(
    ...
    "Aggregate members [region4] not in regional_codes column names [region1, region2, region3]",
    fixed = TRUE
  )
