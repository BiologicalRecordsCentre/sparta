...
```{r traceplots_OTHER, fig.height=0.4*(results$max_year-results$min_year+1), echo = FALSE}
array_sim <- results$BUGSoutput$sims.array
comb.samples <- mcmc.list(
  lapply(1:results$BUGSoutput$n.chains,
         FUN = function(x, array_sim){
...
plot(comb.samples, density = FALSE, smooth = FALSE, )
```
...
barplot(vals,
        ...
        las = 1,
        xlab = 'Rhat',
        main = 'Rhat values (threshold = 1.1)',
        offset = 0,
        width = rep(10, length(cols)),
        yaxs = 'i')
