...
#' @param taxon_name This optional argument gives the name of the data being analysed.
#'        This can be useful if using this function in a loop for a number of datasets
#'        and writing out the results using sinkdir.
...
plr <- 
  function(Data=NULL,#your data (path to .csv or .rdata, or an R object)
    ...
    if(length(taxon_name)>1){warning('taxon_name cannot have length > 1');warn=TRUE}
    ...
    if(is.null(taxon_name) & !is.null(sinkdir)){
      warning("'taxon_name' not given, defaulted to 'plr'. This is used to name your output file")
      taxon_name<-'plr'
    } else if(is.null(taxon_name) & is.null(sinkdir)){
      taxon_name<-'plr'
    }
    
    if(!is.null(sinkdir)) dir.create(sinkdir,showWarnings = FALSE)
          
    print(paste('Starting',taxon_name))
    ...
    for(ii in 1:(length(time_periods[,1])-1)){
      ...
      for(j in (ii+1):length(time_periods[,1])){
        ...
        taxon_temp<-paste(taxon_name,'_',ii,'_',j,sep='')
        ...
        colnames(basic_temp)<-paste(taxon_name,'_',ii,'_',j,sep='')
    ...
    if(!is.null(sinkdir)){  
      file_name<-paste('Basic_trends_',taxon_name,'_',datecode,'.csv',sep='')
      orgwd<-getwd()
      setwd(sinkdir)      
      if (file.exists(file_name)){
        file_name<-paste('Basic_trends_',taxon_name,'_',datecode,'_',format(Sys.time(),'%H%M'),'.csv',sep='')
        warning(paste('Basic_trends_',taxon_name,'_',datecode,'.csv',' already exists.',
