...
function(records, splityr) {
  ...
  n1 <- with(subset(records, year <= splityr), tapply(site, CONCEPT, LenUniq)) # number of sites for each species in t1
  n2 <- with(subset(records, year > splityr), tapply(site, CONCEPT, LenUniq))
