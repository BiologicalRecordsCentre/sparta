...
function(indata, nsp=2, nyr=3, od=F, V=F){ 
  ...
  if(nrow(data)==0){ 
    ...
    MMdata <- dcast(data, year + kmsq ~ ., fun=length, value.var='L') #how many lists for each year?
    names(MMdata)[ncol(MMdata)] <- 'nVis'  
    
    MMdata$nVR <- as.numeric(acast(data, year + kmsq ~ ., fun=sum, value.var='CONCEPT'))
    ...
    if(od){
      ...
      MM <- tryCatch(glmer(cbind(nVR, failures) ~ cYr + (1|kmsq) + (1|obs), data=MMdata, family=binomial, verbose=V))
    } else {
      MM <- tryCatch(glmer(cbind(nVR, failures) ~ cYr + (1|kmsq), data=MMdata, family=binomial, verbose=V))      
    }
    ...
    if(as.numeric(MM@dims['cvg'])==65){ #catches a specific convergence error (not caught above)
      coefs <- c(rep(NA,7),as.numeric(MM@dims['cvg']),length(data$L)/length(indata$L),length(data$L),sum(as.numeric(data$CONCEPT)))
    }
    if(class(MM)[1]!="try-error" & !exists('coefs')){
      coefs <- as.numeric(summary(MM)@coefs[2,])
      coefs <- c(coefs,as.numeric(summary(MM)@coefs[1,1:2]),as.integer(median(unique(as.numeric(MMdata$year)))))
